...
public class MercuryTest {
	...
	public void mercury_tours() throws Exception
	{
		...
		userMercuryDetails=excelutil.getMercuryDetails();
		for(MercuryCredential credential : userMercuryDetails)
		{
			test=report.createTest("Mercury"+credential.getuName());
			...
			//Mercury Register
			regobj.mercuryRe();
			regobj.enterContactDetails(credential.getFirstName(),credential.getLastName(),credential.getPhone(),credential.getEmail());
			regobj.enterMailingDetails(credential.getAddress(),credential.getCity(),credential.getState_province(),credential.getPostalCode());
			regobj.countryDrop();
			regobj.countryDropDetails("INDIA");
			regobj.enterUserInforDetails(credential.getuName(),credential.getPassWord(), credential.getConfirmpassWord());
			driver.manage().timeouts().implicitlyWait(2, TimeUnit.SECONDS);
			screenshot.capture(driver, credential.getFirstName()+"RegisterPage");
			regobj.mSubmitBtn();
			...
			screenshot.capture(driver, credential.getFirstName()+"Successfully Registered");
			driver.manage().timeouts().implicitlyWait(2, TimeUnit.SECONDS);
			regobj.mercuryRHome();
			driver.manage().timeouts().implicitlyWait(2, TimeUnit.SECONDS);
			...
			loginobj.enterLoginDetails(credential.getuName(),credential.getPassWord());
			loginobj.clickLoginBtn();
			screenshot.capture(driver, credential.getuName());
			...
			driver.manage().timeouts().implicitlyWait(2, TimeUnit.SECONDS);
			screenshot.capture(driver, credential.getFirstName()+"FlightFinder");
			...
			driver.manage().timeouts().implicitlyWait(2, TimeUnit.SECONDS);
			screenshot.capture(driver, credential.getFirstName()+"selected Flights");
			selectflightobj.continueBooking();

			// Booking Confirmation 
			bookflightobj.enterBookingDetails(credential.getFirstName(),credential.getLastName(), credential.getNumber());
			bookflightobj.securepurchase();
			screenshot.capture(driver,credential.getFirstName()+"bookingPage");
